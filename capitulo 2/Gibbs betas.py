...
while muest<lim:
  indicador = indicador + 1 # se elige distribución propuesta
  if indicador == 3:
    indicador = 1
  sh=np.shape(cadena) 
...
plt.contourf(np.linspace(ainf,asup,parta),np.linspace(binf,bsup,partb),np.transpose(Z),levels=50)
plt.colorbar().ax.set_ylabel('densidad no normalizada')
#plt.scatter(muestra[0,1:lim+1],muestra[1,1:lim+1],marker='o',color="orange",facecolors='none',label='Muestra',linewidth=1.0)
#plt.legend()
plt.scatter(media_sim[0],media_sim[1],facecolors="r")
#plt.plot(l1,l1,color="r")
plt.show()
...
plt.contourf(np.linspace(ainf,asup,parta),np.linspace(binf,bsup,partb),np.transpose(Z),levels=50)
plt.colorbar().ax.set_ylabel('densidad no normalizada')
plt.scatter(muestra[0,1:lim+1],muestra[1,1:lim+1],marker='.',color="orange",facecolors='none',label='Muestra',linewidth=0.5)
